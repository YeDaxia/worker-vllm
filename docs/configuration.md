...
| `LANGUAGE_MODEL_ONLY`          | `False`             | `bool`                                                      | Skip loading multimodal modules for hybrid models such as Gemma 4 when you only need text generation. |
| `LIMIT_MM_PER_PROMPT`          | `None`              | `str`                                                       | Limit multimodal items per prompt, e.g. `image=1,video=0,audio=0`.              |
...
| `TOOL_CALL_PARSER`                  | `None`      | `str`            | Specifies the parser for tool calls. Options include `mistral`, `hermes`, `llama3_json`, `llama4_json`, `llama4_pythonic`, `granite`, `granite-20b-fc`, `deepseek_v3`, `internlm`, `jamba`, `phi4_mini_json`, `pythonic`, `gemma4`. |
| `REASONING_PARSER`                  | `None`      | `str`            | Parser for reasoning-capable models (enables reasoning mode). Examples: `deepseek_r1`, `qwen3`, `granite`, `hunyuan_a13b`, `gemma4`. Leave unset to disable.                                                                |
